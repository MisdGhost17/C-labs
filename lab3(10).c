...
void transformfile(float *minValue, float *maxValue, int *count) {
    ...
    *count = 0; //счетчик считанных чисел
    ...
    if (*count > 0) {
        int inserted = 0; //флаг для отслеживания вставки минимального значения
        for (int i = 0; i < *count; i++) {
            if (numbers[i] == *maxValue) { //если найдено максимальное значение
                //проверяем, не является ли это первое значение в массиве
                if (i > 0 || !inserted) {
                    ...
                    inserted = 1; //устанавливаем флаг вставки
                }
                break; //выход из цикла после первой вставки
            }
        }
